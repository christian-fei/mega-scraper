...
module.exports = async function preparePage (page, options = {
  ...
  if ((options.blocker || options.images || options.javascript || options.stylesheets) || options.proxy) {
    ...
    page.on('request', async (interceptedRequest) => {
      ...
      if ((options.blocker || options.images || options.javascript || options.stylesheets)) {
      ...
      if (options.proxy) {
        ...
        requestOptions.proxy = `http://${proxy.host || proxy.endpoint}${proxy.port ? `:${proxy.port}` : ''}`
