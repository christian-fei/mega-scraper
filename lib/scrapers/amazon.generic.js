...
module.exports = async function amazonGenericScraper (url, queue) {
  ...
  return paginated({
    ...
    processFn: function ({ queue, events, page }) {
...
async function paginated ({ queue, url, processFn }) {
  if (!url) throw new Error('missing url')
  const events = new EventEmitter()
  const browser = await createBrowser()
  const page = await browser.newPage(url)
  queue.process(processFn({ queue, events, page }))
