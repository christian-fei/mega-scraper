...
module.exports = async function browser ({ useProxy, headless, width, height, slowMo } = { useProxy: process.env.USE_PROXY !== 'false', headless: process.env.HEADLESS !== 'false', width: 1280, height: 800 }) {
  const args = [
    '--no-sandbox',
  ...
  log('new browser', { useProxy, headless, width, height, slowMo }, args)
  return {
    async newPage (url) {
